  ...
  els.feedList.innerHTML =
    state.publicFeed
      .map((f) => {
        ...
        return `<article class="feed-card">${imgWithFallback(f.photoURL, '익명 학습 인증', 'feed-img')}<div class="feed-body"><div class="feed-top"><div style="display:flex;align-items:center;gap:8px">${characterMarkupForStage(f.characterType || 'rabbit', f.characterStage || 1, 'small')}<div class="anon">${safeText(f.anonName || '익명 도전자')}</div></div><span class="tag accent">${Number(f.week)}주차</span></div>${f.punctualClaim ? '<div style="margin-top:8px"><span class="tag yellow">⏰ 정시 실천 배지</span></div>' : ''}<div class="feed-reflection">${safeText(f.reflection || '')}</div><div class="feed-date">인증 시각 ${formatDateTime(submitted)}</div></div></article>`;
      })
...
async function renderInstructorFeed() {
  ...
  const items = buildInstructorFeedItems(students, allSubs, { week: state.selectedFeedWeek, query: instructorSearchQuery });
